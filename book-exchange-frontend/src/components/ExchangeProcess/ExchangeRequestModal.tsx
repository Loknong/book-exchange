...
const ExchangeRequestModal: React.FC<ExchangeRequestModalProps> = ({ isOpen, closeModal }) => {
    ...
    const [selectedSubject, setSelectedSubject] = useState<string>("fiction"); // Default subject filter
    ...
    // Fetch books from OpenLibrary API based on subject
    ...
    useEffect(() => {
        // Fetch books when the modal opens and subject changes
        if (isOpen) {
    ...
    const handleBookClick = (bookTitle: string) => {
        setSelectedBook(bookTitle); // Update selected book state
        setValue("selectedBook", bookTitle); // Set form value for selected book
    };

    const onSubmit = async (data: ExchangeFormInputs) => {
        console.log("Submitting Exchange Offer", data);
        // Handle offer submission logic here (e.g., send to API)
